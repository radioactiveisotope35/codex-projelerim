// update/update-009.js
// Patch 009: Stability, fixed-step loop, pooling, and lifecycle hardening

export function applyPatch(ctx){
  if(!ctx || typeof ctx !== 'object'){
    console.warn('[patch-009] applyPatch requires a context object.');
    return;
  }
  const globalNS = globalThis.__patch009 = globalThis.__patch009 || {};
  if(globalNS.applied){
    return;
  }
  ...
  if(!THREE || !scene || !camera || !renderer || !controls || !clock){
    console.warn('[patch-009] Missing required references.');
    return;
  }
  ...
  const tracerPool = [];
  const activeTracers = [];
  const tracerMaterial = new THREE.LineBasicMaterial({ color: 0xfff6a0, transparent: true, opacity: 0.85 });
  ...
  function initTracerPool(){
    const count = CONFIG.PERF.tracerPoolSize;
  ...
  function spawnTracer(start, end){
    ...
    if(!scene.children.includes(tracer)){
      scene.add(tracer);
    }
  }
  ...
  function initImpactPool(){
    const count = CONFIG.PERF.impactPoolSize;
    for(let i=0;i<count;i++){
  ...
  function spawnImpact(position, normal){
    ...
    tempQuat.setFromUnitVectors(new THREE.Vector3(0,1,0), normal);
    ...
    if(!scene.children.includes(impact)){
      scene.add(impact);
    }
  }
