// update/update-008.js
// Patch 008: Firing gate + recoil removal for Mini CS: Arena

export function applyPatch(ctx){
  if(!ctx || typeof ctx !== 'object'){
    console.warn('[patch-008] applyPatch requires a context object.');
    return;
  }
  if(globalThis.__patch008?.applied){
    return;
  }

  const PATCH_NS = (globalThis.__patch008 = globalThis.__patch008 || {});
  ...
  if(!THREE || !scene || !camera || !controls){
    console.warn('[patch-008] Missing core references, aborting.');
    return;
  }
  ...
  const CONFIG = {
    ...
    STORE: {
      ...
      mods: [
        { id: 'recoil', label: 'Recoil Stabilizer', description: 'Legacy cosmetic (recoil already zero)', apply: () => { playerState.mods.recoil = true; }},
  ...
  PATCH_NS.state = { CONFIG, playerState, fireState };
